...
$null_section = true;
$result = dbquery("SELECT * FROM ".DB_AL_SHOP_CATS." WHERE shp_cat_id='".$_GET['id']."'");
if (dbrows($result)) {
    ...
    $cat = dbarray($result);
    set_title($cat['shp_cat_title']." | ".$locale['shp229']." | ".$settings['sitename']);
    $result = dbquery("SELECT * FROM ".DB_AL_SHOP_CATS." WHERE shp_cat_parent='".$cat['shp_cat_id']."'");
    if (dbrows($result)) {
        $data = make_assoc($result);

        $null_section = false;

        require_once AL_SHOP_TPL_DIR."category_list.php";

    }

    $count = dbcount("(shp_good_id)",DB_AL_SHOP_GOODS,"shp_good_cat='".$cat['shp_cat_id']."' AND shp_good_published='1'");
    ...
    if ($count>0) {
        ...
        $result = dbquery("SELECT * FROM ".DB_AL_SHOP_GOODS." WHERE shp_good_cat='".$cat['shp_cat_id']."' AND shp_good_published='1'".$cost_filter.$filter." LIMIT ".$_GET['rowstart'].",".$shop_settings['goods_per_page']);
...
if ($null_section) {
    require_once AL_SHOP_TPL_DIR."null_category.php";
}
