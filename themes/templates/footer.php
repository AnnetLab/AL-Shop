...
if ($settings['cronjob_hour'] < (time()-360)) {
    ...
    if (dbrows($result)) {
        $result = dbquery("DELETE FROM ".DB_AL_SHOP_BASKET." WHERE shp_basket_user='0' AND shp_basket_date<'".(time()-3*24*3600)."'");
        if (isset($shop_settings) && $shop_settings['currency_manual'] == 0 && $shop_settings['currency_lastupdate'] < time()-24*3600) {
            require_once AL_SHOP_DIR."includes/currencies.php";
            update_currency();
        }
    }
}
